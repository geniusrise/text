...
from geniusrise_text.summarization.fine_tune import SummarizationFineTuner
...
def test_summarization_bolt_init(summarization_bolt, model):
...
def test_load_dataset_all_formats(summarization_bolt, dataset_file, model):
...
def test_summarization_bolt_fine_tune(
    summarization_bolt, dataset_file, model_name, precision, quantization, lora_config, use_accelerate
):
    try:
        tokenizer_name = model_name
        ...
        assert os.path.exists(
            os.path.join(summarization_bolt.output.output_folder, "model", "pytorch_model.bin")
        ) or os.path.exists(os.path.join(summarization_bolt.output.output_folder, "model", "adapter_model.bin"))
        assert os.path.exists(
            os.path.join(summarization_bolt.output.output_folder, "model", "config.json")
        ) or os.path.exists(os.path.join(summarization_bolt.output.output_folder, "model", "adapter_config.json"))
        assert os.path.exists(os.path.join(summarization_bolt.output.output_folder, "model", "training_args.bin"))

        del summarization_bolt.model
        del summarization_bolt.tokenizer
        torch.cuda.empty_cache()

        try:
            os.remove(os.path.join(summarization_bolt.output.output_folder, "model", "pytorch_model.bin"))
            os.remove(os.path.join(summarization_bolt.output.output_folder, "model", "adapter_model.bin"))
            os.remove(os.path.join(summarization_bolt.output.output_folder, "model", "config.json"))
            os.remove(os.path.join(summarization_bolt.output.output_folder, "model", "adapter_config.json"))
            os.remove(os.path.join(summarization_bolt.output.output_folder, "model", "training_args.bin"))
...
def test_summarization_bolt_compute_metrics(summarization_bolt, model):
    name, model_name = model
    tokenizer_name = model_name
    model_class = "AutoModelForSeq2SeqLM"
    tokenizer_class = "AutoTokenizer"
